...
export function collectGitContext(projectRoot, target = "working-tree", ref) {
    ...
    let diffArgs = ["diff", "--no-ext-diff", "--"];
    if (target === "staged")
        diffArgs = ["diff", "--cached", "--no-ext-diff", "--"];
    if (target === "branch")
        diffArgs = ["diff", "--no-ext-diff", `${ref || "main"}...HEAD`];
    if (target === "commit-range")
...
export function readProjectTextFile(projectRoot, filePath, maxChars = 250_000) {
    if (pathIsDenied(filePath))
        throw new Error(`Refusing to read denied path: ${filePath}`);
