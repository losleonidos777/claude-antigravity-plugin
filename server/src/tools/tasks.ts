...
export async function antigravityExecuteTasks(args: any = {}) {
  const projectRoot = getProjectRoot();
  const mode = String(args.mode || "suggest") as "suggest" | "worktree";
  ...
  if (!taskList.trim()) throw new Error("taskListPath or taskListText is required");
...
export async function antigravityVerifyPlan(args: any = {}) {
  const projectRoot = getProjectRoot();
  const plan = args.planText ? String(args.planText) : args.planPath ? readProjectTextFile(projectRoot, String(args.planPath)) : "";
  if (!plan.trim()) throw new Error("planPath or planText is required");
