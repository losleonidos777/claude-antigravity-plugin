...
export async function antigravityReview(args: any = {}) {
  ...
  if (args.background) return launched;
...
export async function antigravityAdversarialReview(args: any = {}) {
  const projectRoot = getProjectRoot();
  const target = args.target || "working-tree";
  const context = collectGitContext(projectRoot, target, args.ref);
  if (!context.hasContent) return skippedReviewResponse();
