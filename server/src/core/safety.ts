...
export function truncate(text: string, maxChars = 500_000): string {
...
export function pathIsDenied(candidate: string, extraDenied: string[] = []): boolean {
  ...
  return rules.some((rule) => {
    ...
    if (r.startsWith("*")) return p.endsWith(r.slice(1));
    if (r.startsWith(".")) return p === r || p.endsWith(r) || p.includes(`/${r}`);
    return p === r || p.endsWith(`/${r}`) || p.startsWith(`${r}/`);
  });
}

export function validatePathPolicy(paths: string[] | undefined, deniedPaths: string[] = []): { ok: boolean; denied: string[] } {
...
export function assertNoDirectEditMode(mode: string): void {
  if (mode === "direct-edit") {
